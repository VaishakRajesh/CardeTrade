...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ==========================================================
    const navbar = document.querySelector('.navbar-glassmorphic, .navbar-premium');
    if (navbar) {
...
// ==========================================================
// 20. LANDING PAGE — Scroll Reveal Animations
//     Uses IntersectionObserver to add 'visible' class
//     when landing sections scroll into view.
// ==========================================================
/*
 * initLandingScrollReveal — IIFE that sets up scroll-triggered
 * reveal animations for landing page sections. Watches elements
 * with .animate-on-scroll-* classes and adds a .visible class
 * when they enter the viewport, then unobserves them.
 */
(function initLandingScrollReveal() {
 const revealEls = document.querySelectorAll(
  '.animate-on-scroll-landing, .animate-on-scroll-left, .animate-on-scroll-right, .animate-on-scroll-scale'
 );
...
// ==========================================================
// 21. LANDING PAGE — 3D Tilt Effect on Feature Cards
//     Subtle perspective tilt on hover for feature cards.
// ==========================================================
/*
 * initTiltCards — IIFE that adds a 3D perspective tilt to
 * .feature-card-landing elements on mouse move, and resets
...
// ==========================================================
// 22. LANDING PAGE — Hero Parallax
//     bg.gif moves slowly when scrolling to create depth.
// ==========================================================
/*
 * initHeroParallax — IIFE that applies a slow parallax shift
 * to the hero background element (.hero-video-bg) based on
 * the current scroll position, creating a depth effect.
 */
(function initHeroParallax() {
 ...
 window.addEventListener('scroll', function() {
  const sy = window.scrollY;
  if (sy < window.innerHeight) {
   heroBg.style.transform = 'translateY(' + (sy * 0.08) + 'px) scale(1.05)';
  }
 }, { passive: true });
})();

// ==========================================================
// 23. LANDING PAGE — Feature Card Counter Animation
//     Numbers count up when stat items scroll into view.
//     Duplicates safeguard: only runs for .hero-stat-item-landing
// ==========================================================
/*
 * initLandingCounters — IIFE that animates number counters
 * inside .hero-stat-item-landing elements. When a counter
 * scrolls into view, it counts up from 0 to the target
 * value using requestAnimationFrame at 60 fps.
 */
(function initLandingCounters() {
 const landingCounters = document.querySelectorAll('.hero-stat-item-landing .counter-value');
 if (landingCounters.length === 0) return;

 landingCounters.forEach(function(el) {
  const text = el.textContent.replace(/[^0-9]/g, '');
  const target = parseInt(el.getAttribute('data-target') || text) || 0;
  if (target === 0) { el.textContent = '0'; return; }
